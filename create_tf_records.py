...
tf.flags.DEFINE_string("train_image_dir", "data_dir/train/",
                       "Training TFRecord directory")
tf.flags.DEFINE_string("val_image_dir", "data_dir/val",
                       "Validation TFRecord directory")

tf.flags.DEFINE_string("train_formulas_file", "data_dir/im2latex_train.lst",
                       "Training formulas LST file.")
tf.flags.DEFINE_string("val_formulas_file", "data_dir/im2latex_test.lst",
...
def _load_and_process_metadata(formulas_file, image_dir):
    """Loads image metadata from a JSON file and processes the formulas.
    Args:
        formulas_file: JSON file containing formula annotations.
...
def main(unused_argv):
    ...
    train_cutoff = int(0.85 * len(val_dataset))
    val_cutoff = int(0.90 * len(val_dataset))
    train_dataset = train_dataset + val_dataset[0:train_cutoff]
    val_dataset = val_dataset[train_cutoff:val_cutoff]
    test_dataset = val_dataset[val_cutoff:]
